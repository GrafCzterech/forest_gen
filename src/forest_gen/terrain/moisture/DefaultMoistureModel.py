...
class DefaultMoistureModel(MoistureModel):
    ...
    def __init__(self, weights: Dict[str, float] | None = None) -> None:
        self.weights = weights if weights is not None else {"flow": 0.5, "slope": 0.3, "aspect": 0.2}

    def compute(self, flow: np.ndarray, slope: np.ndarray, aspect: np.ndarray) -> np.ndarray:
        ...
        moisture = w_flow * flow_norm + w_slope * slope_penalty + w_aspect * aspect_factor
