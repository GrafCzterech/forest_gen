...
def generate_traversability_map(
    ...
    Z_interp = RegularGridInterpolator((base_grid, base_grid), 
                                       mesh.vertices[:, 2].reshape(size, size))
            ...
            for j in range(high_res_size):
                ...
                if indices:
                    ...
                    for idx in indices:
                        ...
                        penalty_value = tree_penalty * (1 - 0.3 * (d / tree_influence_radius))
    ...
    traversability = (slope_weight * slope_score + (1 - slope_weight) * alt_score) * tree_map
