...
class TerrainGenerator:
    ...
    def generate(self, config: TerrainConfig) -> None:
        ...
        self.slope, self.aspect = SlopeAspectCalculator(config.resolution).compute(hm)
        self.moisture = self.moisture_model.compute(self.flow, self.slope, self.aspect)
        self.heightmap = hm
    ...
    def show_flow(self) -> None:
        if self.flow is None:
            raise RuntimeError("Flow accumulation not computed. Call generate() first.")
    ...
    def show_heightmap(self) -> None:
        if self.heightmap is None:
            raise RuntimeError("Heightmap not generated. Call generate() first.")
