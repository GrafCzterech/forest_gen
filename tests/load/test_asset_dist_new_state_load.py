...
def _make_species(
    ...
    *,
    ...
    # Keep vt() predictable; new_state ignores it anyway but we want consistency.
    return Species(
        name=name,
        max_age=200,
        radius=radius,
...
def _assert_clearance_two_groups(big_pts, small_pts, big_r: float, small_r: float):
    """
    Your sim.py clearance rule is max(r1, r2), so for big/small it's:
      - big-big >= big_r
      - small-small >= small_r
      - big-small >= big_r
    Uses KDTree for O(N log N) checks.
    """
    ...
    if len(big_pts) > 1:
        tree = KDTree(big_pts)
        d, _ = tree.query(big_pts, k=2)  # nearest neighbor is itself at 0
...
def test_new_state_load_high_density_multispecies(sym):
    ...
    # Expected samples per species ~= floor(scene_density * density * area)
