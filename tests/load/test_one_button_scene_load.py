...
def test_one_button_world_load_profile(sym):
    ...
    obstacle_density = float(os.getenv("LOAD_OBS_DENSITY", "0.005"))  # ~size^2*density
    ...
    # Deterministic randomness for anything that uses `random` internally.
    random.seed(123)
    np.random.seed(123)

    # ---- resolve symbols (allow flexible layouts) ----
    ...
    # Prefer FractalNoise for heavier workload; fall back to Simplex if layout differs.
    try:
    ...
    # Understory + Grass (years=0; reproduction disabled)
    ...
    # Traversability (base + obstacle-penalized)
    ...
    # Local check: nearest cell to an obstacle should be penalized
    ...
    # Coordinate system sanity across all generated assets
