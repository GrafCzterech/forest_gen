...
def test_run_state_ages_and_spawns_in_bounds(sym):
    ...
    # After one year:
    # - original plant aged from 1 -> 2
    # - newly spawned plants are age 0
...
def test_run_state_respects_max_population_cap(sym):
    ...
    sp = _species(
        ...
        name="Cap",
        max_age=200,
        radius=0.03,              # tiny radius => fewer overlaps => easier to hit cap
        reproduction_rate=8,
...
def test_run_state_removes_plants_older_than_max_age(sym):
    ...
    size = (10.0, 10.0)
    # age > max_age => should be removed immediately on run_state
