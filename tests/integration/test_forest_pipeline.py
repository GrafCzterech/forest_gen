...
def _species(Species, *, name="Canopy", max_age=50, radius=0.8, species_density=0.002):
    # Keep simulation deterministic-ish: no reproduction, constant viability.
    return Species(
        name=name,
        max_age=max_age,
        radius=radius,
        species_density=species_density,
        reproduction_rate=0,          # IMPORTANT: avoids random seeding during years>0
...
def test_forest_builder_generate_years_respects_bounds_even_with_small_size(sym):
    """
    Regression guard: small scenes + PoissonDisk sometimes expose boundary bugs.
    """
    pytest.importorskip("scipy")
    ...
    size = (12.0, 12.0)
    sp = _species(Species, radius=0.6, species_density=0.01)  # still feasible
