...
def test_flow_accumulator_flat_heightmap_all_ones(sym):
    ...
    # On a perfectly flat surface, every cell is a sink -> accumulation should be 1 everywhere.
...
def test_flow_accumulator_monotone_slope_outlet_collects_all_cells(sym):
    ...
    # Strictly decreasing toward bottom-right corner => all cells should drain there.
    hm = np.array(
        [
            [9, 8, 7],
            [6, 5, 4],
    ...
    # In correct D8 flow accumulation: outlet should have total cell count (9 here).
...
def test_slope_aspect_flat_surface_slope_zero(sym):
    ...
    # Flat -> slope should be 0 everywhere.
...
def test_aspect_flips_by_180_degrees_when_ramp_reverses(sym):
    ...
    # Compare circular difference; should be ~180 regardless of absolute convention.
...
def test_drainage_carver_reduces_height_where_flow_is_high(sym):
    ...
    flow[2, 2] = 10.0  # peak flow at center

    strength = 0.3
    carver = DrainageCarver(strength=strength, sigma=0.0)
    out = carver.apply(hm, flow)

    # With sigma=0, carving should be exact at the peak.
...
def test_drainage_carver_clips_to_unit_interval(sym):
    ...
    hm = np.full((4, 4), 2.0, dtype=np.float32)  # out-of-range input
    ...
    assert np.allclose(out, 1.0)  # clipping dominates (no carving because flow_norm==0)
