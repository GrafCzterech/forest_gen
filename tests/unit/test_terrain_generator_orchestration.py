...
def test_terrain_generator_skips_microrelief_when_disabled(sym, monkeypatch):
    ...
    # Stub hydrology/derivatives so this test stays unit-level.
...
def test_terrain_generator_applies_height_scale_and_wires_arrays(sym, monkeypatch):
    ...
    # Fixed outputs for the patched pipeline steps.
    ...
    class _DC:
        def apply(self, hm, f):
            # ensure generator passes the flow we expect
            assert f is flow
            return hm
    ...
    class _SAC:
        ...
        def compute(self, hm):
            # slope/aspect should be computed after height scaling
    ...
    base_hm = np.full((3, 3), 0.5, dtype=np.float32)
    micro_out = np.full((3, 3), 0.5, dtype=np.float32)  # keep it simple
    noise = _NoiseStub(base_hm)
    ...
    # Height scaling should be applied after carving.
    assert float(t.heightmap[0, 0]) == pytest.approx(1.0)
    assert t.heightmap.shape == (cfg.rows, cfg.cols)

    # Moisture model must be called with the outputs of the pipeline.
    f_arg, s_arg, a_arg = moisture_stub.args
    assert f_arg is flow
    assert s_arg is slope
    assert a_arg is aspect

    # Returned Terrain stores the moisture result.
