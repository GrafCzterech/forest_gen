...
def test_new_state_points_in_bounds_and_no_intraspecies_conflicts(sym):
    """
    Real check against SciPy PoissonDisk: output plants must be within bounds
    ...
    sp = Species(
        ...
        max_age=50,
        radius=2.0,
        species_density=0.01,  # area=900 -> desired_n=9
        reproduction_rate=0,
    ...
    # Intra-species clearance: PoissonDisk radius should guarantee >= sp.radius
    plants = list(state)
...
def test_new_state_prioritizes_larger_radius_species_when_conflicts(sym, monkeypatch):
    """
    This tests the real design contract: species are sorted by radius descending.
    ...
    big = Species(
        ...
        max_age=10,
        radius=5.0,
        species_density=0.01,  # floor(1.0) -> 1 sample
        reproduction_rate=0,
        juvenile_mortality_depth=0.0,
        juvenile_recovery_age=0.2,
        viability_map=lambda x, y: 1.0,  # noqa: E731
...
def test_new_state_uses_clearance_radius_semantics_max_not_sum(sym, monkeypatch):
    """
    Locks in your current semantics: 'radius' is a clearance requirement,
