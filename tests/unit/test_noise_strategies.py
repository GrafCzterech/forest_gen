...
def test_simplex_noise_size_zero_should_not_produce_nan(sym):
    ...
    # What we want long-term: either a valid [0,1] map or a clean exception.
    assert np.isfinite(hm).all()
...
def test_fractal_noise_deterministic_and_normalized(sym):
    ...
    # Preserve global RNG state because implementation uses np.random.seed().
...
def test_fractal_noise_should_not_mutate_global_numpy_rng(sym):
    ...
    # Ideal behavior: generating should not reset/affect global RNG stream.
    np.random.seed(999)
