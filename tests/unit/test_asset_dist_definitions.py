...
def test_viability_map_is_bounded_and_deterministic_when_seed_is_forced(sym, monkeypatch):
    ...
    # Force deterministic internal seed used by ViabilityMap via random.randint patch.
    ...
    assert 0.0 <= v1 <= 1.0
    assert 0.0 <= v2 <= 1.0
    # With forced seed, instances should match.
...
def test_plant_vt_juvenile_spike_reduces_viability_near_peak(sym):
    ...
    sp = Species(
        ...
        juvenile_mortality_peak=0.05,   # around age 5
    ...
    # At age=5 (the peak), vt should be lower than at age=10 for these params.
    assert p5.vt() < p10.vt()
...
def test_plant_vt_prim_applies_spatial_viability_and_population_weight(sym):
    ...
    plant = Plant(coords=(1.0, 2.0), species=sp, age=20)  # vt ~= 1

    pop = {sp: 5}
    total = 20

    val = plant.vt_prim(pop, total)
    # vt is ~1.0 here, so vt_prim ~ vmap * (a/sum_a) = 0.25 * 0.25 = 0.0625
...
def test_seed_does_not_exceed_reproduction_radius(sym):
    ...
    # This configuration should logically constrain seeds to <= reproduction_radius,
    # but current code draws distance from uniform(species_density, reproduction_radius).
    sp = Species(
        name="SeederBug",
        max_age=10,
        reproduction_rate=10,
        reproduction_radius=2.5,
        species_density=10.0,  # WRONG UNITS, exposes the bug clearly
        juvenile_mortality_depth=0.0,
        juvenile_recovery_age=0.2,
    )
    ...
    random.seed(0)
    seeds = list(p.seed())
    assert seeds  # ensure we actually sampled some
